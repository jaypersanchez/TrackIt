...
public class IdenMIDlet extends MIDlet implements CommandListener 
{
    ...
    public IdenMIDlet() {
    	...
    	try {
    	    ...
    	    //rse.printStackTrace();
            display.setCurrent( new Alert("IdenMIDlet", "error:"+rse.getMessage(),null, AlertType.INFO));
    	}
    	...
    	/**/if (numOfRecords == 0) {
    		...
    		try {
    			...
    			//e.printStackTrace();
                    display.setCurrent( new Alert("IdenMIDlet", "error:"+e.getMessage(),null, AlertType.INFO));
    		}
